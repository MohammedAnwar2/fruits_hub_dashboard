...
class AdvertisementListViewBlocConsumer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return BlocConsumer<ViewAndDeleteAdvertisementCubit,
        ViewAndDeleteAdvertisementState>(
      listener: (context, state) {
        if (state is DeleteAdvertisementSuccess) {
          showEffectEventMessage(context, 'Advertisement Deleted Successfully');
        }
        if (state is ViewAndDeleteAdvertisementFailure) {
          ...
          case ViewAdvertisementLoading():
            return Skeletonizer(
              ...
              child: AdvertisementListViewItem(
                  advertisements: dummyAdvertisements),
            );
